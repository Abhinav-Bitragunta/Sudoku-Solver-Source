...
class Board {
   ...
   public:
    ...
    inline void                 set                 (const size_t& idx, const int& val);
    inline size_t               gridNum             (const size_t& row, const size_t& col);
...
inline size_t Board::gridNum(const size_t& row, const size_t& col) {
    size_t gridSize = sqrt(this->N);
    return (row/gridSize) * gridSize + (col/gridSize);
}

inline const int& Board::at(const size_t& row, const size_t& col)              {   return board[row * this->N + col];  }
inline void Board::set(const size_t& row, const size_t& col, const int& val)   {   board[row * this->N + col] = val;   }
